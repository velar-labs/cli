...
import { VelarComponentMeta } from "../../types/index.js";
import {
  IConfigManager,
  IFileSystemService,
  IRegistryService,
...
describe("ComponentService", () => {
  ...
  beforeEach(() => {
    ...
    mockConfigManager = {
      getComponentsPath: vi
        .fn()
        .mockReturnValue("resources/views/components/velar"),
      validate: vi.fn(),
      load: vi.fn(),
  ...
  describe("addComponents", () => {
    it("should add multiple components successfully", async () => {
      ...
      mockRegistryService.resolveDependencies.mockResolvedValue([
        mockComponent,
      ]);
    ...
    it("should handle failures for specific components", async () => {
      mockRegistryService.fetchComponent.mockRejectedValue(
  ...
  describe("destinations", () => {
    it("should return correct destination for CSS", async () => {
      const compWithCss = {
        ...mockComponent,
        files: [{ type: "css", path: "button.css" }],
      };
    ...
    it("should return default destination for unknown type", async () => {
      const compWithOther = {
        ...mockComponent,
        files: [{ type: "other", path: "other.txt" }],
      };
  ...
  describe("file conflicts", () => {
    ...
    it("should cancel if user chooses to", async () => {
      ...
      expect(result.failed).toHaveLength(1);
      expect(result.failed[0]?.error).toBe("exit");
    });
  });
